...
check('All thirteen personalities resolve to registered instruments',()=>{
  assert.equal(Object.keys(VISUAL_PROFILES).length,13)
...
check('Profile selection applies era, instruments and media defaults',()=>{
  settings().setVehiclePresetById('jzx100');assert.equal(settings().display.clusterLayout,'JDM_GT_93');assert.equal(media().visualStyle,'EQ_DECK_89')
...
check('Ignition-off stores coordinates and settings/media survive rehydration',()=>{
  ...
  settings().setVehiclePresetById('minimpi');settings().updateDisplay({nightMode:true});media().setVisualStyle('CD_TUNER_95')
  useSettingsStore.persist.rehydrate();useVehicleStore.persist.rehydrate();useMediaStore.persist.rehydrate()
  assert.equal(settings().vehicleProfile.id,'minimpi');assert.equal(settings().display.nightMode,true);assert.equal(media().visualStyle,'CD_TUNER_95');assert.equal(vehicle().parkedLocation?.lon,79.8612)
})
...
check('All profile artwork, phone homes, GPS families and seven media skins render',()=>{
  ...
  for(const style of ['CASSETTE_86','EQ_DECK_89','DSP_RECEIVER_92','CD_TUNER_95','MD_DOT_MATRIX_98','HERITAGE_RADIO','EXPEDITION_RECEIVER'] as const){media().setVisualStyle(style);assert.ok(renderToStaticMarkup(<MediaScreen/>).includes(style==='HERITAGE_RADIO'?'Heritage Radio':media().currentTrack().title))}
})
